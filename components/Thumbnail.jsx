...
const Thumbnail = forwardRef(({ result }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="p-2 group cursor-pointer transition-all duration-200 ease-in transform sm:hover:scale-105 hover:z-50"
    >
      <div className=" bg-slate-700 rounded-md">
        <div className={!data && "animate-pulse rounded-md"}>
          ...
          />
          <div className="p-2">
            <p className={`${bgLoading} h-6 truncate rounded-md max-w-md`}>
              {data.overview}
            </p>

            <h2
              className={`${bgLoading} h-8 mt-1 text-2xl rounded-md text-white transition-all duration-100 ease-in-out group-hover:font-bold`}
            >
